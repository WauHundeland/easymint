...
export function MintSelect({ value, onValueChange, onTokenSelect, placeholder = "Select a token mint..." }: MintSelectProps) {
  ...
  const fetchUserMints = async () => {
    ...
    try {
      ...
      // Process mint infos and filter for user's mint authority using manual parsing
      ...
      for (let i = 0; i < allMintInfos.length; i++) {
        ...
        try {
          ...
          // Filter for tokens where user has the mint authority
          if (parsedMint.mintAuthorityOption === 0) continue;
          const mintAuthority = new PublicKey(parsedMint.mintAuthority).toString();
          if (mintAuthority !== publicKey.toString()) continue;
    ...
    <div className="space-y-2">
            ...
            <SelectContent>
              {tokens.length === 0 && !loading ? (
                <SelectItem value="no-mints" disabled>
                  No mints found where you have mint authority
                </SelectItem>
              ) : (
                tokens.map((token) => (
      ...
      {tokens.length === 0 && !loading && !error && (
        <Alert>
          ...
          <AlertDescription>
            No token mints found where you have mint authority. Create a token first to mint additional tokens.
          </AlertDescription>
        </Alert>
      )}
    </div>
